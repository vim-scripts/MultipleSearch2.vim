...
"Set to 1 if you want to search N n the searched patterns
let g:ManageVimSearch=1
"after starting vim sometime we need continue with searching last 
let g:KeepOldVimSearch=1
...
function! s:MultipleSearchInit()
    ...
    if exists('g:MultipleSearchColorSequence')
        ...
        let s:ColorSequence = "yellow,red,blue,green,magenta,darkcyan,gray,lightgreen"
    endif
    ...
    if exists('g:MultipleSearchTextColorSequence')
        ...
        let s:TextColorSequence = "black,white,white,black,white,black,black,black"
    endif
...
" current buffer.
" -----
function! s:InitSearchStrings()
	let g:AllWords=""
endfunction

function! s:AddStringtoSearch(string)
	if !(exists('g:AllWords'))
		let g:AllWords=""
	endif

	if !(a:string=="")
		if !(g:AllWords=="")
			let g:AllWords=g:AllWords."\\|".a:string
		else
			let g:AllWords=a:string
		endif
	endif
endfunction
...
function! s:DoSearch(useSearch, forwhat)
	...
	if g:ManageVimSearch==1
		if !(exists('g:AllWords'))
				call s:InitSearchStrings()
				endif
		if a:forwhat==""
				call s:DoReset()
				return 0
		else
				if g:KeepOldVimSearch==1
					if !(g:AllWords == @/)
							" Highlight the vim old search
					  execute 'syntax match MultipleSearch7 "' . @/ . '" containedin=ALL'
					endif
					call s:AddStringtoSearch(@/)
				endif

				call s:AddStringtoSearch(a:forwhat)	
				let @/=g:AllWords
				exe "/"
		endif
	endif
